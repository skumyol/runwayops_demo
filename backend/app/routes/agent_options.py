...
async def get_agent_options(
    ...
    skip_prediction: bool = Query(
        False,
        description="Skip predictive agent (use when disruption already confirmed)"
    ),
) -> dict[str, Any]:
    ...
    2. Runs rebooking agent (skips prediction if disruption confirmed)
    3. Auto-includes hotel if delay > 3 hours
    ...
    Args:
        ...
        skip_prediction: If True, skips predictive agent (for confirmed disruptions)
    ...
    logger.info(f"   Skip Prediction: {skip_prediction}")
    ...
    if disruption:
        ...
        delay_minutes = disruption.impact.get("delayMinutes", 0) if hasattr(disruption, 'impact') and disruption.impact else 0
        logger.info(f"⏱️  Delay: {delay_minutes} minutes")
    else:
        delay_minutes = 0
    
    # If skip_prediction is True and we have a disruption, generate options directly
    if skip_prediction and disruption:
        logger.info("⏭️  Skipping predictive agent - disruption already confirmed")
        logger.info("🚀 Step 3: Generating rebooking options directly")
        
        # Calculate if hotel needed (delay > 3 hours = 180 minutes)
        hotel_required = delay_minutes > 180
        logger.info(f"🏨 Hotel required: {hotel_required} (delay: {delay_minutes}min)")
        
        # Generate options directly using rebooking tool
        from ..agentsv2.tools import rebooking_tool
        
        rebooking_result = await rebooking_tool(
            flight_id=flight_number,
            pax_count=manifest.summary.affectedCount,
            delay_minutes=delay_minutes,
            vip_count=int(manifest.summary.affectedCount * 0.1),
            disruption_type=disruption.type,
            origin="HKG",
            destination="LAX"  # Would extract from flight data
        )
        
        # Build analysis structure for option generation
        analysis = {
            "disruption_detected": True,
            "risk_assessment": {
                "risk_probability": 1.0,  # 100% - disruption confirmed
                "pax_impact": "high" if manifest.summary.affectedCount > 100 else "medium",
                "reasoning": f"Confirmed disruption: {disruption.type}"
            },
            "rebooking_plan": rebooking_result,
            "finance_estimate": {
                "total_usd": rebooking_result.get("total_cost_usd", 0),
                "rebooking_cost": rebooking_result.get("rebooking_cost_usd", 0),
                "hotel_meals_cost": rebooking_result.get("hotel_cost_usd", 0) if hotel_required else 0,
                "compensation_usd": rebooking_result.get("compensation_usd", 0)
            },
            "crew_rotation": {
                "status": "Available",
                "crew_available": True
        ...
        logger.info("✅ Rebooking options generated directly")
    else:
        # Build input for full agent workflow
        input_data = {
            "airport": "HKG",
            "carrier": "CX",
        ...
        try:
            ...
            logger.info("🚀 Step 3: Launching full agent workflow")
            logger.info("-" * 80)
